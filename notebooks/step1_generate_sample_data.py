...
# MAGIC
# MAGIC 小売企業向けMedallionアーキテクチャ構築 - Bronze層への取り込み前提データを生成します。
# MAGIC
# MAGIC **含まれるデータソース:**
# MAGIC - `pos_transactions` : POS取引データ（ニアリアルタイム想定）
# MAGIC - `product_master`   : 商品マスター（バッチ更新）
# MAGIC - `store_master`     : 店舗マスター（バッチ更新）
# MAGIC - `customer_master`  : 顧客ロイヤルティデータ（バッチ更新）
...
txn_rows = [
    ...
    # [品質課題①] transaction_id の重複 (TXN-003 が2行)
...
# COMMAND ----------
# MAGIC %md
# MAGIC ## 4. 意図的に含めたデータ品質問題一覧

# COMMAND ----------

from pyspark.sql.functions import (
    count, when, isnull, isnan, col, lit
)
...
df_dup = (
    ...
    .select(
        col("transaction_id"),
        lit("① transaction_id の重複").alias("issue_type"),
        col("cnt").cast("string").alias("detail"),
    )
)
...
df_no_store = (
    ...
    .select(
        col("transaction_id"),
        lit("② store_id の欠損").alias("issue_type"),
        lit("store_id = NULL").alias("detail"),
    )
)

# ---------- ③ 商品マスター未存在 product_id ----------
df_unknown_product = (
    df_pos
    .join(df_product, on="product_id", how="left_anti")
    .select(
        col("transaction_id"),
        lit("③ 商品マスターに存在しない product_id").alias("issue_type"),
        col("product_id").alias("detail"),
    )
)
...
df_no_user = (
    ...
    .select(
        col("transaction_id"),
        lit("④ user_id の欠損").alias("issue_type"),
        lit("user_id = NULL").alias("detail"),
    )
)
...
df_unknown_user = (
    ...
    .join(df_customer, on="user_id", how="left_anti")
    .select(
        col("transaction_id"),
        lit("⑤ 顧客マスターに存在しない user_id").alias("issue_type"),
        col("user_id").alias("detail"),
    )
)
...
df_neg_qty = (
    ...
    .select(
        col("transaction_id"),
        lit("⑥ quantity がマイナス").alias("issue_type"),
        col("quantity").cast("string").alias("detail"),
    )
)
...
df_null_price = (
    ...
    .select(
        col("transaction_id"),
        lit("⑦ unit_price が NULL").alias("issue_type"),
        lit("unit_price = NULL").alias("detail"),
    )
)
...
df_issues = (
    df_dup
    .union(df_no_store)
    .union(df_unknown_product)
    .union(df_no_user)
    .union(df_unknown_user)
    .union(df_neg_qty)
    .union(df_null_price)
