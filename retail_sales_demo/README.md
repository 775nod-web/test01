...
`app/` 配下に、Serving layer一式のソースコード（バックエンドAPI／React(TS)フロントエンド／ガバナンス用SQL／アラート・再照合バッチ／`app.yaml`）を格納しています。**このコードはDatabricksワークスペースに未接続の状態で生成したもの**で、カタログ/スキーマ名やSDKレスポンス構造などは実環境での検証が必要です。詳細は `app/README.md` の「デプロイ前チェックリスト」を参照してください。

## 9. 実装の進め方（Databricksワークスペースに接続する場合）

Databricksワークスペースに接続されたコーディングエージェントが使える場合は、`prompts/` 配下のプロンプトを**フェーズ順に**投入することで、`app/`のコードを実環境の値に合わせて検証・調整しながら進められます。各プロンプトは実行結果として「コード」と「ビジネス目的への整合性・使用サービスの説明」の両方を出力するよう指示しています。
