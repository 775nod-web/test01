...
- ヘッダーにデータモード・モデルモード・最終更新を常時表示（生成方式は顧客ごとの性質上、次のアクションパネル内に表示）（※その後、利用者の依頼によりヘッダーのデータモード・モデルモードバッジは削除。本README 6節「ヘッダーのデータモード・モデルモードバッジを削除した経緯」参照）
...
- Step 6に、施策後の反応・利用状況から次の推奨アクション（固定8カテゴリのルールベース判定）を追加し、結果確認だけで終わらない画面にした（`backend/services/feedback_action_service.py`、`recommended_next_action`フィールド、本README 6節「次の改善アクション」）。あわせて追加した全顧客集計の「今回の改善ポイント」は、どの顧客の結果か分からない・カード内の理由と内容が重複するとの利用者からの指摘を受けて削除した
...
追加当初は、見出し「施策後の反応と利用状況（デモ用サンプル）」・サンプルである旨の注記・判断の活用目的を説明する文の3箇所でサンプルであることを明示していたが、その後利用者の依頼によりいずれも一度削除した。次の推奨判断機能を追加するにあたり、利用者から改めてサンプルである旨の注記を追加する依頼があり一度再表示したが、直後に利用者から再度削除の依頼があり、`frontend/src/components/FeedbackSummaryPanel.tsx`から「施策後の反応、利用状況、次の推奨判断は、改善ループを説明するためのデモ用サンプルです。」という注記を削除した。**現時点で、画面上にサンプルである旨の明示は一切ない。** バックエンドの`sample_outcomes`レスポンスには引き続き`is_sample: true`が含まれており、API上では判別できる。

バックエンドの`sample_outcomes`レスポンスには引き続き`is_sample: true`が含まれており、API上でも常にサンプルであることを判別できる。

### 次の改善アクション（Step 6：顧客ごとの推奨アクション）

Step 6を結果確認だけで終わらせず、「続ける施策・見直す施策・次に検証する内容」を判断できる画面にするため、施策後の反応・利用状況から次の推奨アクションをデモ用の説明可能なルールベースで導出し、各顧客カードに表示する。
...
追加当初は、各顧客の推奨アクション種別を「継続・展開候補」「見直し候補」「次回検証する仮説」の3つに集計し、代表1件から1文を生成する「今回の改善ポイント」ブロックをカード一覧の下に表示していた（`backend/services/feedback_action_service.py`の`build_improvement_summary()`、`GET /api/feedback-summary`の`improvement_summary`フィールド）。

利用者から、この表示について「どの顧客の話か分からない」「カード内の『施策後の反応』『理由』と内容が重複している」という指摘があった。実際、この集計は複数顧客を統合した分析ではなく、各カテゴリの代表1件（該当する顧客のうち最初に見つかったもの）の`customer_response`と推奨ラベルを定型文に当てはめているだけであり、顧客名を明記していなかったため、指摘は妥当と判断した。カード単位の表示（次の推奨判断・理由・次回確認）だけでStep 6の目的は十分満たせるため、`build_improvement_summary()`・`improvement_summary`フィールド・対応するUIブロック・関連テストを削除した。
...
### ヘッダーのデータモード・モデルモードバッジを削除した経緯

**この変更は、CLAUDE.mdの必須表示要件と明確に衝突するものであり、他のUI文言削除とは性質が異なることをここに明記する。**

Phase 4までは、ヘッダーに以下2つのバッジを常時表示していた。

- データモードバッジ：「合成データ（demo）」または「Databricks接続」（`metadata.data_mode`に対応）
- モデルモードバッジ：「事前計算済み予測」または「実学習モデル」（`metadata.model_mode`に対応）

この表示は、CLAUDE.mdの次の要件に対応するものだった。

- 4節「Databricks」：「合成データ利用中であることをUIとREADMEで明示する」
- 9節「受け入れ条件」：「合成データが明示される」「生成方式とモデル方式が明示される」

利用者からの依頼により、この2つのバッジを`frontend/src/App.tsx`の`HeaderMeta`コンポーネントから削除した。削除にあたり、これが上記のCLAUDE.md必須要件と衝突する旨を利用者へ明示的に提示し、利用者はそれを承知の上で削除を希望したため実施した。

**現時点で、ヘッダーには合成データである旨・モデルモードである旨の表示が一切ない。** 最終更新時刻のみが表示されている。ただし、次の2箇所には引き続き同種の情報が残っている。

- 中央下「③ 休眠リスク」パネルの脚注（`frontend/src/components/RiskPanel.tsx`）：「事前計算済み予測 ・ モデルバージョン ... ・ 推論日時 ...」という表示は、この変更の対象に含まれておらず、削除していない（DEMO_SPEC.md 8節が要求するモデルバージョン・推論日時の表示のため）。
- APIレスポンス（`GET /api/metadata`の`data_mode`/`model_mode`フィールド）自体は変更しておらず、引き続き`demo`/`precomputed`等の値を返す。判別はAPIレベルでのみ可能で、画面上では判別できない。

本番化した場合に実データ・実LLM学習モデルへ切り替わっても、この画面だけを見て「デモかどうか」を利用者が判断する手段はない。この点は本README 25節「既知の制約一覧」にも記載する。
...
`feedback-summary` エンドポイントの `sample_outcomes` は、代表顧客6名分の`customer_id`・`display_name`・`campaign_status`・`customer_response`・`usage_recovery_status`・`observed_at`・`is_sample: true`・`recommended_next_action`（`type`/`label`/`reason`/`next_review_timing`）に加え、該当顧客の実際の最新判断があれば`decision`・`selected_action`・`comment`・`decided_at`（無ければ全てnull）を含む（本README 6節「フィードバックループ」「次の改善アクション」参照）。全顧客を集計した`improvement_summary`フィールドは、利用者からの指摘により一度追加した後に削除している（本README 6節「全体改善サマリー（今回の改善ポイント）の削除について」参照）。

## 17. アプリの使い方（Phase 1〜6時点）

Phase 1〜4を通じて、Layer 1〜4の一連の業務フロー（Step①〜⑥）が一画面で完結します。画面各所の見出しには①〜⑥の番号を付け、デモを進める順序がひと目でわかるようにしています。

- ヘッダー：アプリ名、最終更新時刻。データモード・モデルモードのバッジは利用者の依頼により削除済み（本README 6節「ヘッダーのデータモード・モデルモードバッジを削除した経緯」参照）
...
1. アプリを開く。ヘッダーのデータモード・モデルモードバッジは利用者の依頼により削除済みのため、これがデモ用の合成データであることは口頭で補足する（本README 6節「ヘッダーのデータモード・モデルモードバッジを削除した経緯」参照）。
...
- ヘッダーのデータモード・モデルモードバッジを利用者の依頼により削除したため、画面を見ただけでは合成データ利用中であることが判別できない（APIレスポンスの`data_mode`/`model_mode`では判別可能。本README 6節「ヘッダーのデータモード・モデルモードバッジを削除した経緯」参照）。CLAUDE.md 4節・9節が求める「合成データの明示」を、この画面は満たしていない。
...
- [x] ヘッダーにデータモード・モデルモード・最終更新が常時表示される（Phase 4時点。※その後、利用者の依頼によりデータモード・モデルモードバッジは削除。本README 6節参照）
...
- [ ] フロントエンドで「施策後の反応と利用状況」がサンプルである旨の画面表示：追加当初→利用者の依頼で削除→次の推奨判断機能の追加時に利用者の依頼で再表示→直後に利用者の依頼で再度削除、という経緯を経て、現時点では画面上にサンプルである旨の明示は一切ない（本README 6節「サンプルである旨の画面表示の変遷」参照）。APIレスポンスの`is_sample: true`では判別可能
...
- [ ] 合成データが明示される：デプロイ後、利用者の明示的な依頼によりヘッダーの「合成データ（demo）」バッジを削除したため、この時点では画面上に合成データである旨の明示が無く、条件を満たしていない。この削除はCLAUDE.md 4節・9節の必須表示要件と衝突する旨を提示した上で、利用者が承知の上で削除を希望したため実施した（本README 6節「ヘッダーのデータモード・モデルモードバッジを削除した経緯」参照）。APIレスポンス（`GET /api/metadata`の`data_mode`）では引き続き判別可能
