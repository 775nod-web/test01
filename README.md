...
7. 下部「⑥ フィードバック概要」で、件数と直近の判断一覧に今保存した判断が反映されていることを確認する。
8. 時間があれば、最下部「デモ構成／本番化時に追加する事項」を開き、デモで実装済みの範囲と本番化時に追加する範囲の違いを説明する。
...
- [x] 本番未実装機能が明示される（「デモ構成／本番化時に追加する事項」の折りたたみパネル、本README 24節）
