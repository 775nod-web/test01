...
貼り付けて実行してください。セクションごとに複数セルへ分割して実行すると、
前のセクションで作成した df_stores 等の変数が定義されていないセルを
単独実行した際に NameError: name 'df_stores' is not defined のような
エラーになります（クラスター再起動やノートブック再アタッチでも
Pythonの変数状態は失われるため、同様のエラーが再発します）。
そのため生成〜保存までの処理全体を1つの関数にまとめ、
このファイルを1セルとして実行すれば必ず最初から最後まで
一貫した状態で実行されるようにしています。
...
def generate_and_save_retail_sample_data():
    """
    店舗マスター・商品マスター・会員マスター・POSトランザクションの
    サンプルデータを生成し、sampleスキーマへ保存する。

    生成〜保存までを1つの関数にまとめることで、途中の変数（df_stores等）
    が他のセルの実行状態に依存しないようにしている（NameError対策）。
    """
...
generate_and_save_retail_sample_data()
