...
- 「直近N日/Nヶ月」のようなデフォルト期間は、実行環境の `date.today()` / SQLの `current_date()` ではなく、**対象テーブルに実在する最新日付（`MAX(kpi_date)` 等）を基準**に計算すること。サンプル/デモデータは実際のカレンダー日付と無関係な期間で作られることがあり、実機で「クエリは成功するが0件」という不具合を実際に起こしたため。
